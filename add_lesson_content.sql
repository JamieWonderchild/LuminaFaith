-- LuminaFaith: Additional Lesson Content
-- Execute this in your Supabase SQL Editor to add more comprehensive lesson content

-- 1. Add more lessons to Christianity - Introduction path
...
-- 5. Add a sample daily challenge
INSERT INTO daily_challenges (challenge_date, type, title, description, content, xp_reward) VALUES
(CURRENT_DATE, 'reflection', 'Gratitude Practice', 'Reflect on three things you are grateful for today', 
'{"prompt": "Take a moment to think about three things you are grateful for today. They can be big or small - a loving relationship, a warm meal, a beautiful sunset, or simply being alive. Write them down and reflect on why they bring you joy.", "questions": [{"id": "q1", "type": "reflection", "question": "What are three things you are grateful for today?", "reflection_prompt": "Take time to truly feel gratitude for each item on your list."}]}', 
25);

-- Update the total_lessons count for all paths
...
-- Update the total_paths count for all religions
UPDATE religions 
SET total_paths = (
    SELECT COUNT(*) 
    FROM learning_paths 
